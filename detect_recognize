import cv2
import mediapipe as mp
from insightface.app import FaceAnalysis
import numpy as np
import os, json
from datetime import datetime

# MediaPipe setup
mp_drawing = mp.solutions.drawing_utils
mp_face_detection = mp.solutions.face_detection

# InsightFace setup
face_recognizer = FaceAnalysis(name="buffalo_l", providers=["CPUExecutionProvider"])
face_recognizer.prepare(ctx_id=-1)

# Load known embeddings
def load_known_faces(path="known_faces"):
    db = {}
    for file in os.listdir(path):
        if file.lower().endswith((".jpg", ".png")):
            img_path = os.path.join(path, file)
            img = cv2.imread(img_path)
            if img is None:
                print(f"[WARNING] Unable to read {file}")
                continue
            faces = face_recognizer.get(img)
            if faces:
                name = os.path.splitext(file)[0]
                print(f"[INFO] Loaded: {name}")
                if name not in db:
                    db[name] = []
                db[name].append(faces[0].normed_embedding)
            else:
                print(f"[WARNING] No face found in {file}")
    return db

known_faces = load_known_faces()

# Compare embeddings
def recognize_face(embedding, threshold=0.75):
    best_match = "Unknown"
    best_score = -1
    for name, embeddings in known_faces.items():
        for known_emb in embeddings:
            score = np.dot(embedding, known_emb) / (
                np.linalg.norm(embedding) * np.linalg.norm(known_emb)
            )
            if score > best_score and score > threshold:
                best_match = name
                best_score = score
    return best_match, best_score

# Webcam and detection
cap = cv2.VideoCapture(0)
with mp_face_detection.FaceDetection(model_selection=0, min_detection_confidence=0.5) as detector:
    while cap.isOpened():
        success, frame = cap.read()
        if not success:
            print("Ignoring empty frame.")
            continue

        image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = detector.process(image)
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

        if results.detections:
            for detection in results.detections:
                bbox = detection.location_data.relative_bounding_box
                h, w, _ = image.shape

                # Add margin around the face box
                margin = 30
                x1 = max(0, int(bbox.xmin * w) - margin)
                y1 = max(0, int(bbox.ymin * h) - margin)
                x2 = min(w, x1 + int(bbox.width * w) + 2 * margin)
                y2 = min(h, y1 + int(bbox.height * h) + 2 * margin)

                # Crop face and validate
                face_crop = image[y1:y2, x1:x2]
                if face_crop.size == 0 or face_crop.shape[0] < 50 or face_crop.shape[1] < 50:
                    print("[DEBUG] Invalid crop, skipping...")
                    continue

                # Resize for InsightFace input
                face_crop_resized = cv2.resize(face_crop, (112, 112))

                # Detect face inside the cropped image
                recog_faces = face_recognizer.get(face_crop_resized)
                print(f"[DEBUG] Detected {len(recog_faces)} face(s) in cropped image")

                name = "Unknown"
                score = 0.0
                if recog_faces:
                    emb = recog_faces[0].normed_embedding
                    name, score = recognize_face(emb)

                # Draw box and label
                cv2.rectangle(image, (x1, y1), (x2, y2), (0, 255, 0), 2)
                cv2.putText(image, f"{name} ({score:.2f})", (x1, max(20, y1 - 10)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

                # Save log
                log = {
                    "PersonID": name,
                    "DateTime": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "Confidence": round(float(score), 2)
                }
                with open("detections.json", "a") as f:
                    f.write(json.dumps(log) + "\n")


        # Show webcam feed
        cv2.imshow("Face Detection + Recognition", image)
        if cv2.waitKey(5) & 0xFF == 27:
            break

cap.release()
cv2.destroyAllWindows()
